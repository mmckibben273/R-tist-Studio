...
```{r}
filter(data1, !species == "Adelie" & body_mass_g > 3400) #Another way to filter out a species is by putting an exclamation mark in front of the column name. Here we filter out all of the data points for Adelie penguins and datapoints for penguins with a body mass less than 3400 g
...
```
Some of the Star Wars cells have lists. For example, some characters were in multiple films. To be able to plot these categories, we need to unnest the lists.
...
Make a bar chart showing the number of characters that appear in each Star Wars film.
