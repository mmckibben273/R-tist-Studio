...
```{r}
data2 <- iris
bw <- (2 * IQR(data2$Petal.Width)) / (length(data2$Petal.Width)^(1/3)) #This equation uses the Freedman-Diaconis rule (FD) to give give you a good estimate for which binwidth to choose.
ggplot(data2, aes(x = Petal.Width, fill=Species)) +
  geom_histogram(binwidth = bw)+ #Try changing the bin width and rerunning the code to see how it changes your visualization. When deciding binwidths for your histograms, make sure to think about the range/length of your x axis and how your counts are distributed.
  theme_classic()
```
...
```{r}
ggplot(data = data2) +
  aes(x = Petal.Length, y = Petal.Width, color=Species) + #In this line we tell ggplot to color the points by the flower species
  xlab("Petal Length")+
  ylab("Petal Width")+
  geom_smooth(se=FALSE)+
  ggtitle("Petal Width by Petal Length")+
  geom_point() +
  theme_classic()
```
